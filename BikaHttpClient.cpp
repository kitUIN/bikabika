...
namespace winrt::bikabika::implementation
{
	...
	hstring BikaHttpClient::Authorization()
	{
		return m_authorization;
	}
	void BikaHttpClient::Authorization(hstring const& value)
	{	
	...
	hstring BikaHttpClient::SetRaw(hstring strAPI, hstring uid, time_t t, hstring method, hstring apiKey)
	{   // 原始URL地址
	...
	hstring BikaHttpClient::BikaEncryption(hstring strAPI, hstring uid, time_t t, hstring method, hstring apiKey, hstring strKey)
	{   //HMAC-SHA256签名验证
	...
	HttpRequestHeaderCollection BikaHttpClient::SetHeader(HttpRequestHeaderCollection headers, hstring strAPI, guid uuid, time_t t, hstring method)
	{   //生成请求头
	...
	HttpResponseMessage BikaHttpClient::GET(Uri requestUri, hstring strAPI, guid uuid, hstring apiKey, hstring strKey)
	{   //GET类型
	...
	HttpResponseMessage BikaHttpClient::POST(Uri requestUri, HttpStringContent jsonContent, hstring strAPI, guid uuid)
	{   //POST类型
		HttpClient httpClient;
		HttpRequestMessage httpRequestMessage;
		HttpResponseMessage httpResponseMessage;
		...
		httpRequestMessage.Headers().Authorization(HttpCredentialsHeaderValue(L"  "));
	...
	HttpResponseMessage BikaHttpClient::Login(hstring account, hstring password)
	{   //登陆获取token
